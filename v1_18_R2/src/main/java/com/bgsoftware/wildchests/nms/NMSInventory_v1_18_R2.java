...
import static com.bgsoftware.wildchests.nms.NMSMappings_v1_18_R2.*;

@SuppressWarnings({"unused", "ConstantConditions"})
public final class NMSInventory_v1_18_R2 implements NMSInventory {
    ...
    private static class TileEntityWildChest extends TileEntityChest implements IWorldInventory, TileEntityContainer, BlockEntityTicker<TileEntityWildChest> {
        ...
        public Container a(int id, PlayerInventory playerinventory) {
            Container container = NMSInventory_v1_18_R2.createContainer(id, playerinventory,
        ...
        public void tick(World world, BlockPosition blockPosition, IBlockData iBlockData, TileEntityWildChest tileEntity) {
            ...
            {
                double x = getX(this.o) + this.n.v.nextFloat();
                double y = getY(this.o) + this.n.v.nextFloat();
                double z = getZ(this.o) + this.n.v.nextFloat();
    ...
    public static class WildInventory implements IInventory {
        ...
        public void setItem(int i, WildItemStack<?, ?> wildItemStack, boolean setItemFunction){
            ...
            if(!ItemStack.a(original.getItemStack(), itemstack)) {
                if (NMSMappings_v1_18_R2.isEmpty(itemstack))
                    nonEmptyItems--;
                else
                    nonEmptyItems++;
            }

            if (!NMSMappings_v1_18_R2.isEmpty(itemstack) && this.getMaxStackSize() > 0 && getCount(itemstack) > this.getMaxStackSize()) {
